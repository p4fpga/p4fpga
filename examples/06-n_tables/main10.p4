...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    apply {
        ...
        if (hdr_0.dpl.repeat > 8w0) 
            apply_dummy_tables_dummy_9.apply();
        if (hdr_0.dpl.repeat > 8w0) 
            apply_dummy_tables_dummy_10.apply();
        if (hdr_0.dpl.repeat > 8w0) 
            apply_dummy_tables_dummy_11.apply();
        if (hdr_0.dpl.repeat > 8w0) 
            apply_dummy_tables_dummy_12.apply();
        if (hdr_0.dpl.repeat > 8w0) 
            apply_dummy_tables_dummy_13.apply();
        hdr = hdr_0;
        meta = meta_0;
