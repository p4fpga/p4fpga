...
from llvmType import LLVMType
from compilationException import CompilationException
from programSerializer import ProgramSerializer
import typeFactory


class LLVMInstanceBase(object):
    def __init__(self):
        pass


class SimpleInstance(LLVMInstanceBase):
...
class LLVMHeader(SimpleInstance):
    """ Represents a header instance from a P4 program """
    def __init__(self, hlirHeaderInstance, factory):
        super(LLVMHeader, self).__init__(hlirHeaderInstance, factory, False)
...
class LLVMMetadata(SimpleInstance):
    ...
    def __init__(self, hlirMetadataInstance, factory):
        super(LLVMMetadata, self).__init__(hlirMetadataInstance, factory, True)
...
class LLVMHeaderStack(LLVMInstanceBase):
    ...
    def __init__(self, hlirInstance, indexVar, factory):
        super(LLVMHeaderStack, self).__init__()

        # indexVar: name of the llvm variable that
        # holds the current index for this stack
        assert isinstance(indexVar, str)
        assert isinstance(factory, typeFactory.LLVMTypeFactory)
        ...
        assert isinstance(self.basetype, LLVMType)
