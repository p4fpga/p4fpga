...
def generate_parse_body(builder, json, bbmap, structmap, node, stack, visited=None):
    ''' walk parser tree '''
    if visited is None:
        visited = set()
    visited.add(node.name)
    stack.append(node.name)
    generate_parse_state(builder, node, structmap, json.parser[node.name])
    for block in node.control_state.basic_block:
        if type(block) == str:
            continue
        next_header = bbmap[block[1]].name
        if next_header not in visited:
            generate_parse_body(builder, json, bbmap, structmap, bbmap[block[1]], stack, visited)
    stack.pop()
...
def generate_deparse_body(builder, json, bbmap, node, stack, visited=None):
    ''' walk deparser tree '''
    if visited is None:
        visited = set()
    visited.add(node.name)
    stack.append(node.name)
    generate_deparse_state(builder, json.deparser[node.name])
    for block in node.control_state.basic_block:
        if type(block) == str:
            continue
        next_header = bbmap[block[1]].name
        if next_header in visited:
            continue
        generate_deparse_body(builder, json, bbmap, bbmap[block[1]], stack, visited)
    stack.pop()
...
class BSVControlFlow(ControlFlow):
    ...
    def generate_deparser(self, builder, json):
        ...
        builder.append(generate_deparse_state_enum(json))
        generate_deparse_idle(builder)
        generate_deparse_body(builder, json, self.basic_blocks, start_block, [], set())
        builder.append(generate_deparse_top(0, json))

    def generate_parser(self, builder, json):
        ''' TODO: move dfs to serialize '''
        stack = []
        visited = set()
        start_block = self.basic_blocks[self.control_state.basic_block[0]]
        ...
        builder.append(generate_parse_state_enum(json))
        builder.append(generate_parse_state_init(json))
        generate_parse_body(builder, json, self.basic_blocks, self.structs, start_block, [])
        builder.append(generate_parse_top(visited, json))

    def generate_control_flow(self, builder, json):
        ''' buidl control flow json '''
        builder.append(generate_control_flow_top(self, json))

    def bsvgen(self, builder, json):
        ''' generate control flow from json '''
        assert isinstance(builder, SourceCodeBuilder)
        if self.name == 'parser':
            self.generate_parser(builder, json)
        elif self.name == 'deparser':
            self.generate_deparser(builder, json)
        else:
            self.generate_control_flow(builder, json)
