...
class LLVMDeparser(object):
    ...
    def serializeHeaderEmit(self, header, serializer, program):
        ...
        if llvmProgram.LLVMProgram.isArrayElementInstance(p4header):
            llvmStack = program.getStackInstance(p4header.base_name)
            ...
            llvmHeader = program.getHeaderInstance(p4header.name)
